...
export default function CustomerChatPage() {
  ...
  // Initialize on load
  // We'll use an effect to check auth when the component mounts
  // but we need to be careful about nextjs hydration
...
export default function CustomerChatPage() {
  ...
  async function handleAction(action: any) {
    ...
    if (action.type === 'view_alerts') {
      router.push('/admin/alerts') // This would need to be created
      return
    }

    if (action.type === 'view_stats') {
      router.push('/admin/stats') // This would need to be created
      return
    }
  ...
  function renderAction(action: any, idx: number) {
    ...
    if (action.type === 'find_jobs') {
      // This is more relevant for worker persona, but we'll keep it for completeness
      return (
        <button
          key={idx}
          type="button"
          onClick={() => {
            // Navigate to worker dashboard or job search
            router.push('/worker')
          }}
          disabled={isLoading}
          className="block text-left text-xs bg-white/20 rounded px-2 py-1 hover:bg-white/30 disabled:opacity-50"
