-- ============================================
-- Manual payout: processing state + confirm step
-- ============================================

-- RPC: Approve payout (set to processing, don't touch wallet)
CREATE OR REPLACE FUNCTION public.approve_payout_processing(
  p_payout_id UUID
) RETURNS JSONB
...
-- RPC: Confirm payout (manual complete after bank transfer)
...
BEGIN
  ...
  IF v_payout.status NOT IN ('processing') THEN
    RETURN jsonb_build_object('success', false, 'error', 'Payout not in processing state');
  END IF;
  ...
  RETURN jsonb_build_object('success', true, 'payout_id', p_payout_id, 'status', 'completed');
END;
$$;
