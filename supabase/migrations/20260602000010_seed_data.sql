...
-- === PROFILES ===
INSERT INTO profiles (id, email, phone, role, full_name, is_active, created_at)
SELECT
  gen_random_uuid()::text,
  format('user%s@vifixa.test', i),
  format('09%09d', floor(random() * 1000000000)::int),
...
INSERT INTO workers (user_id, full_name, skills, is_verified, trust_score, avg_earnings, completed_orders, rating_avg)
SELECT
  id,
  full_name,
...
-- === SERVICE AREAS (10 HCMC districts) ===
INSERT INTO service_areas (worker_id, name, center_lat, center_lng, radius_km, geometry, is_active)
SELECT
  (SELECT user_id FROM workers ORDER BY random() LIMIT 1),
  name,
  lat,
  lng,
  radius,
  jsonb_build_object('type','Polygon','coordinates',jsonb_build_array(coords)),
  true
FROM (VALUES
  ('Quận 1',    10.7769, 106.7009, 3.0, '[[106.690,10.770],[106.710,10.770],[106.715,10.785],[106.690,10.785],[106.690,10.770]]'),
...
BEGIN
  FOR i IN 1..5 LOOP
    ...
    IF o_id IS NOT NULL THEN
      ...
      INSERT INTO complaints (order_id, customer_id, reason, status, created_at)
      VALUES (o_id, c_id,
        ...
        (ARRAY['pending','under_review','resolved'])[floor(random() * 3 + 1)],
